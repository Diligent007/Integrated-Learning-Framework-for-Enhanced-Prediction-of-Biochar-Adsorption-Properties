...
def print_data(model_name, file_name, y_test, y_pred):
    ...
    sorted(y_pred) = sorted(y_pred).sample(len(sorted(y_pred)))
...
def train_model(model, model_name, file_name, val_data, val_labels):
    ...
    joblib.dump(model, r"D:\博士期间所有文件汇总\陈超文章\吸附性-共热解-调研\数据试跑\数据开启\Yield+BET\集成学习\明白后集成\推进-1\回归正常\2\同等级别\model\{}.pkl".format(model_name))
